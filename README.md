...
- 桌面工具儀表板依台北日期統計**各帳號連續簽到天數**（同一天任一成功時段即算）
...
每個帳號只要設定其中一種登入方式即可，且 Storage State 優先。**未設定 Secret 的帳號會直接略過，不會進入錯開等待**（不佔用延遲時間）。
...
因此若帳號 1 在 `T` 開始，帳號 *n* 約在 `T + Σ(5…15)` 秒後開始；前後帳號可重疊執行（不需等前一個完全跑完），但不會同時點擊。**未設定登入 Secret 的帳號會立即結束、不 sleep**，只有已設定的帳號才套用上述錯開延遲。GitHub 的排程本身也可能延遲，因此實際開始時間可能略晚於上述時段。
...
- **找不到每日領取按鈕**：到 workflow 的 Artifact 查看截圖。目前 UI 是右上角開啟帳戶／盒飯選單後，點「每日簽到領額外盒飯」旁的 **+20**（文案可能含「升級會員」，腳本會依「每日簽到」意圖辨識，不會誤點購買／訂閱）。**已領取**時同一位置會變成 **「明天見！」**，腳本會視為今日已成功。若 OiiOii 改版，可能需要再調整 `scripts/claim-lunch.mjs`。
